...
- UAC: различимые сообщения для кодов `1223` (пользователь отклонил запрос) и `740` (повышение заблокировано политикой) — локализованы во все 6 языков; раньше оба показывались как «неизвестная ошибка».
- Мьютекс: `main()` освобождает handle в `finally` даже при аварийном выходе из mainloop — важно для elevate-цепочки (новый процесс ждёт до 60 сек).
